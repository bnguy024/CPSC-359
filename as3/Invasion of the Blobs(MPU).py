...
# 30009135 CPSC 359 FALL 2020 
# Implements MPU acelerometer for movement, shoot, menu and pause functionality
# Still keeps the orginal code, for debugging purposes (keyboard is still functional)

import pygame, os, random
from pygame.locals import *
from mpu6050 import mpu6050 #Import mpu to use the accelerometer 

random.seed()
WORLD = Rect(0, 0, 480, 550)
sensor = mpu6050(0x68) #Accessing the 0x68 register 
...
class Game:
    ...
    def pauseLoop(self):
        ...
        while self.paused:
            # Implement of accelerometer to unpause the game once the Y axis gets below 5 
    ...
    def menuLoop(self):
        ...
        while 1:
            ...
            self.clock.tick(60)
            accel_data = sensor.get_accel_data()
# Implement of accelerometer in the start screen to select start game
            if accel_data['y'] > 0:
                option = 1
                if accel_data['z'] > 15:
                    self.gameLoop()
# Implement of accelerometer in the start screen to select exit game                      
            if accel_data['y'] < 0:
                option = 2
                if accel_data['z'] > 15 and option == 2:
                    pygame.quit()
                    return
    ...
    def gameLoop(self):
        ...
        while 1:
            ...
            self.bg.update()
            self.pauseLoop()
# Implement of accelerometer to move the space ship using the x axis greater than 0 moves the spaceship right less than 0 moves left
            if accel_data['x'] < 0:
                self.ship.rect.move_ip(-5, 0)
            if accel_data['x'] > 0:
                self.ship.rect.move_ip(5, 0)

# Implement of accelerometer to get the spaceship to shoot once the Z axis gets past 15
            if accel_data['z'] > 15 and not self.ship.overheated :
                self.ship.heat += 0.75
                if self.ship.reload_timer >= self.ship.reload_time:
                    self.ship.reload_timer = 0
                    Shot(self.ship.rect.midtop)
                    if self.ship.poweredup:
                        Shot(self.ship.rect.midtop, 1)
                        Shot(self.ship.rect.midtop, -1)
                    self.ship.shoot_sound.play()
            else:
                if self.ship.heat > 0:
                    self.ship.heat -= 1

            if self.ship.poweredup:
                self.ship.powertimer -= 1
                if self.ship.powertimer <= 0:
                    self.ship.poweredup = False

            if self.ship.heat >= 100:
                self.ship.overheated = True
            if self.ship.overheated:
                self.ship.heat -= 1
                if self.ship.heat <= 0:
                    self.ship.overheated = False
# Implement of accelerometer to pause the game once the Y axis gets past 5     
            if accel_data['y'] > 5:
                    self.paused ^= 1
