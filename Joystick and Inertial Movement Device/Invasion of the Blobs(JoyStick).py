...
# 30009135 CPSC359 FALL 2020
# Implements joystick as movement, button press for shoot and select. Movement on X axis for left and right movement
# Y axis movements for menu select and pausing/unpausing game
# Keeps the original implementation of code (Keyboard is still functional), for debugging
...
#Make functions of the buttons, taken from tutorial 5 joystick_pos_update.py
def isButton(b):
    if (b == 1):
        return True
    else: return False
...
class Game:
    ...
    def pauseLoop(self):
        ...
        while self.paused:
            ...
            #Press down on joystick to unpause game 
            if directionY == -1.0:
                self.paused ^= 1
    ...
    def menuLoop(self):
        ...
        while 1:
            ...
            #Lights for up and down direction
            if (high(y)):
                yellow.on()
                directionY = 1.0
            else:
              ...
              #Selects up for the menu 
            if directionY == 1.0:
                option = 1
            
            #Selects down for the menu         
            if directionY == -1.0:
                option = 2

            #Quit game         
            if option == 2 and  button == 1:
                    pygame.quit()
                    return
            #Start game 
            if option == 1 and button == 1:
                    self.gameLoop()
    ...
    def gameLoop(self):
        ...
        #Initialize the variables need to be used 
        positionX = 0.0
        directionX = 0
        directionY = 0
        ...
        while 1:
            ...
            #Turns on the lights for each direction of the joystick 
            if (high(x)):
                blue.on()
                directionX = 1.0
            else:
            ...
            # Make sure that the joystick voltage is correct
            
            deltaX = abs(2.6 - x) / 2.6
            ...
            #Impletements button to shoot if pressed     
            if button == 1 and not self.ship.overheated :
                self.ship.heat += 0.75
            ...
            # If y axis is pressed up then the game is paused      
            if directionY == 1.0:
                     self.paused ^= 1 
