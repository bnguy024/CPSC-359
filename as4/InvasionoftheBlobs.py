...
# Implementation of the us100 for left and right movement of the ship during gameplay
#code for the assignment starts at line 486
...
class Game:
    ...
    def gameLoop(self):
        ...
        while 1:
            ...
            self.bg.update()
            self.pauseLoop()
            #Code from tutorial 5 modified for the game
            sensor.TX_set() 
            time.sleep(0.0001)
            sensor.TX_clear() 
            while (sensor.RX_read() == 0):
                time.sleep(0.00001)
            #get time from BCMTimer
            t1 = timer.getTimer()
            while (sensor.RX_read() == 1):
                time.sleep(0.00001)
            #get time from BCMTimer
            t2 = timer.getTimer()
            timePeriod = (t2 - t1 ) /1000000
            #Format the distance as a whole number for readability 
            distance = int((timePeriod * 34300) / 2)
            print("distance:", distance)
            #Deadzone where the ship doesnt move is between 25mm and 15mm
            if (distance <= 25 & distance >=15):
                self.ship.rect.move_ip(0, 0)
            #Move ship left
            if (distance >15):
